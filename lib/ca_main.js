...
exports.newInstance = function($, spec, cb) {
    try {
        ...
        var blockCreate = spec.env.blockCreate;
        delete spec.env.blockCreate;
        blockCreate &&  $._.$.log && $._.$.log.warn('Blocking CA create ' +
                                                    spec.name);

        var that = genCA.constructor($, spec);
        that.$.ca = that;
        var getLeaseTimout = function () {
        ...
        var leaseTimeout = getLeaseTimout(); // in seconds
